...
<body id="thietbi">
    ...
    <section class="equipment-list">
        <div class="equipment-card">
            <p class="equipment-info">ID: <span>${thietbi.equipmentID}</span></p>
            <h2 class="equipment-name">Tên thiết bị: ${thietbi.name}</h2>
            <img class="equipment-img" src="${thietbi.img}" alt="${thietbi.name}">
            <p class="equipment-info">Mô tả: <span>${thietbi.description}</span></p>
            <p class="equipment-info">Trạng Thái: <span>${thietbi.status}</span></p>
            <p class="equipment-info">Số lượng: <span>${thietbi.quantity} Cái</span></p>
            <form action="/demospringmvc/thietbi/delete" method="post" onsubmit="return confirm('Bạn có chắc chắn muốn xóa thiết bị này không?');">
                <input type="hidden" name="equipmentID" value="${thietbi.equipmentID}">
