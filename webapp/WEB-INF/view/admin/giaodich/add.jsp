...
<form action="/demospringmvc/giaodich/add" method="post">
    <label for="customerID">Mã khách hàng:</label>
<select id="customerID" name="customerID">
    <c:forEach var="itemkh" items="${khachhang}">
        <option value="${itemkh.customerID}">${itemkh.customerID}</option>
    </c:forEach>
</select>
    <label for="amount">Tổng giao dịch:</label>
<select id="amount" name="amount">
    <c:forEach var="itemgt" items="${goitap}">
        <option value="${itemgt.price}">${itemgt.price}</option>
    </c:forEach>
</select>

    <label for="description">Chi tiết:</label>
    <input type="text" id="description" name="description">
    <label for="transactionDate">Ngày thực hiện giao dịch:</label>
