...
<section class="registration-list">
    <c:forEach var="item" items="${dangkiloptap}">
        ...
        <h2 class="registration-ID">ID: ${item.registrationID}</h2>
            ...
            <form action="/demospringmvc/dangkyloptap/update" method="post">
                <input type="hidden" id="registrationID" name="registrationID" value="${item.registrationID}">
                <input type="hidden" id="customerID" name="customerID" value="${item.customerID}">
                <label for="scheduleID">Lớp tập khách hàng muốn thay đổi:</label><br>
                <select id="scheduleID" name="scheduleID">
                    <c:forEach var="itemlt" items="${lichtap}">
                        <option value="${itemlt.scheduleID}" <c:if test="${item.scheduleID == itemlt.scheduleID}">selected</c:if>>${itemlt.className}</option>
                    </c:forEach>
                </select><br>
