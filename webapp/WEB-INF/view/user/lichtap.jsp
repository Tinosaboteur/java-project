...
<body id="lichtap">
  ...
  <h1>Chào mừng đến với phòng tập Gym của chúng tôi!</h1>
	...
	<div class="back-home">
        ...
        <a class="transaction-link" href="/demospringmvc/lichtap"><h1>Lịch tập</h1></a>
    </div>
	
	
	
	<section class="schedule-list">
    <c:forEach var="item" items="${lichtap}">
        <div class="schedule-card">
            <h2 class="class-name">${item.className}</h2>
            <img class="class-img" src="${item.img}" alt="${item.className}">
            <p class="class-info">Mô tả: <span>${item.description}</span></p>
            <p class="class-info">Ngày bắt đầu: <span>${item.date}</span></p>
            <p class="class-info">Giờ bắt đầu: <span>${item.startTime}</span></p>
            <p class="class-info">Giờ kết thúc: <span>${item.endTime}</span></p>
            <c:forEach var="itemnv" items="${nhanvien}">
                <c:if test="${item.employeeID == itemnv.employeeID}">
                    <p class="class-info">Nhân viên phụ trách: <span>${itemnv.name}</span></p>
                </c:if>
            </c:forEach>
        </div>
    </c:forEach>
